...
A rebuild of the original [Multi-Agent-Voice-Enabled-Shopping-Assistant](https://github.com/thinkhp5/Multi-Agent-Voice-Enabled-Shopping-Assistant)
retargeted to deploy on **Amazon Bedrock AgentCore Runtime**, with tools
exposed as an **MCP server** instead of local Python functions.

The LangGraph orchestration (orchestrator → parallel product/support agents →
synthesizer, with HITL interrupts) is unchanged in shape. What changed:
...
Both halves deploy independently via the `agentcore` CLI. The agent side
never imports tool implementations directly — it only knows the MCP server's
URL, exactly like it would for a third-party MCP server.
...
See [`DEPLOY.md`](./DEPLOY.md) for the full `agentcore configure` / `agentcore deploy`
sequence for both the MCP server and the agent, including how to wire the
agent's `MCP_SERVER_URL` to the deployed MCP server's Runtime ARN.
